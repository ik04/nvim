...
vim.api.nvim_set_keymap('i', '<C-v>', '<ESC>"+pi', { noremap = true, silent = true, desc = "Paste from system clipboard (Insert Mode)" })

-- 4. Ctrl+V for Paste from System Clipboard (in Visual Mode)
-- Replaces the selected text with content from the system clipboard.
vim.api.nvim_set_keymap('v', '<C-v>', '"+p', { noremap = true, silent = true, desc = "Paste from system clipboard (Visual Mode)" })
